...
**MCP tools:** `scout(query, top_k=5)` in `src/mcp_server.py`, returning `{items, jobs}`. `items` is backed by `brain.search_links_scoped()` — owner-scoped semantic search over the caller's own links, reusing `/find`'s tuned `0.58` relevance bar (`_SCOUT_MIN_SCORE`) instead of the looser `BRAIN_MIN_SCORE` to keep noise down. `jobs` is backed by `brain.search_jobs_scoped()` (shipped 2026-09-19, #645) — substring match on title/url **plus an exact tag-name match** (added 2026-09-22, unioning `job_tags` with `link_tags` via `jobs.link_id` the way `count_jobs_by_tag` does, since a swept job's tags live on its link), so a job carrying a tag with no title/url overlap still surfaces. `items` stays semantic-only — no tag matching there; use `list_items` with a tag name for exact link matches. `get_scout_settings()` returns the caller's autonomy toggle so the agent knows whether it may search unprompted.
...
**Status: shipped (2026-09-19, #644).** All ten mutation tools plus the read surface (`list_spaces`, `get_space_detail`) are live in `src/mcp_server.py`. `list_items` and `get_space_detail`'s pinned jobs both surface `drive_url` (#648, 2026-09-21) — the link's/job's own Obsidian note in Drive, or null if not yet uploaded.
...
This is not "ship all four, see what sticks." Phase 2 is complete — its infra blocker was already resolved by Phase 1's embeddings work, so it shipped without waiting on usage data. Phase 3 shipped without the usage-data validation this roadmap called for — a deliberate deviation, not evidence the gate was wrong for the next roadmap that follows this shape. Phase 4 is a different object (Spaces, not Links) rather than a continuation of the Brain-cleanup lineage, but follows the same phase discipline: propose-then-approve for anything destructive or judgment-based, narrow tool surface per operation, no scope creep into adjacent concerns (job ingestion) without its own grilling session. Phase 4 is also now shipped, so all four planned phases are live.

**Current tool inventory (`src/mcp_server.py`, 2026-09-22):** `list_items`, `get_item_detail`, `delete_item`, `list_tags`, `find_related` (Phase 1/2); `get_scout_settings`, `scout` (Phase 3); `list_spaces`, `get_space_detail`, `create_space`, `update_space`, `delete_space`, `add_space_url`, `remove_space_url`, `reorder_space_url`, `create_context_blob`, `update_context_blob`, `delete_context_blob`, `reorder_context_blob` (Phase 4) — 19 tools total.

`list_tags()` (added 2026-09-22) wraps the existing `database.list_tags`/`count_jobs_by_tag` primitives — each tag with a `job_count` (jobs + swept-to-link jobs, deduped). Added because there was no way to enumerate a caller's tag vocabulary short of paging every `list_items` result and collecting `tags` client-side. `job_count` only counts jobs, not raw link attachments — a tag applied only to links (never a job) reads as `job_count: 0` even though it's in active use; if that undercount turns out to matter, add a `link_count` alongside it rather than conflating the two.

No tool exists yet to list or filter *jobs* by tag directly (only `scout`'s substring+tag-name match, or a Brain link's tags via `list_items`); flagged as a gap, not yet its own phase.
