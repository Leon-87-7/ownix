...
**Current tool inventory (`src/mcp_server.py`, 2026-09-22):** `list_items`, `get_item_detail`, `delete_item`, `find_related` (Phase 1/2); `get_scout_settings`, `scout` (Phase 3); `list_spaces`, `get_space_detail`, `create_space`, `update_space`, `delete_space`, `add_space_url`, `remove_space_url`, `reorder_space_url`, `create_context_blob`, `update_context_blob`, `delete_context_blob`, `reorder_context_blob` (Phase 4) — 18 tools total. No tool exists yet to list or filter *jobs* by tag directly (only `scout`'s substring+tag-name match, or a Brain link's tags via `list_items`); flagged as a gap, not yet its own phase.
