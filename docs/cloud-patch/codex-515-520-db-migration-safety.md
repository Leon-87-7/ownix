...
**Current state.** `init_db` (`src/database.py:1550`) opens the connection, sets
`PRAGMA journal_mode=WAL` (`database.py:1558`), determines `is_fresh`
(`database.py:1563`), and on the **non-fresh** path runs a destructive links-dedup
`DELETE` (`database.py:1570-1596`) and `executescript(SCHEMA_SQL)` (`database.py:1597`)
**before** calling `_run_migrations` (`database.py:1603`). `_run_migrations`
(`database.py:1443`) reads `PRAGMA user_version`, then for each pending step applies
it and commits a bumped `user_version` per step (`database.py:1462-1464`). Fresh
installs branch away entirely and stamp `user_version = len(_MIGRATIONS)`
(`database.py:1600`). Nothing is ever backed up. The only backup guidance in the repo
...
**Current state.** If a step in `_run_migrations` (`database.py:1443`) raises, earlier
steps have already committed their `user_version` bumps (`database.py:1462-1464`),
...
(`database.py:509`, which does `DROP TABLE jobs` at `database.py:523`) and the
destructive dedup deletes at `database.py:964`, `database.py:1170`, and
`database.py:1320`. There is no convention requiring a rollback note or a
two-phase drop.
