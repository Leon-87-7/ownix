...
2. **Ground it with `brand-lens`.** Invoke the `brand-lens` skill with the concrete defect (step 1's before/after) as its target, not the general area. Its own completion criterion applies — a 👍/👎 verdict per applicable Law. Every solution proposed in step 4 must trace to one of those verdict lines, not "seems fine."

3. **If any verdict is 👎, grill it to agreement.** A 👎 is real tension, not a compromise to rank alongside cleaner options — the plan underneath it needs to change shape, not get outvoted. Invoke the `grilling` skill with the defect and brand-lens's 👎 lines as the plan under stress: each 👎 is a branch on the design tree, its Law the stakes of that branch, and its 👍 sibling lines are settled context the tree doesn't need to revisit. Work the tree in rounds to grilling's own completion criterion — frontier empty, user confirms shared understanding — then re-run `brand-lens` against the resulting design. If new 👎s surface, grill again; repeat until a `brand-lens` run comes back at full agreement (zero 👎), or the user explicitly accepts a residual tension by name — don't keep grilling past a tension they've already chosen to accept. Skip this step entirely when step 2 already came back clean.

4. **Propose solutions, cheapest first, then stop.** Grep/read the actual component(s) before proposing — every option must name a real file, and reuse an existing component/prop/pattern already in the codebase over inventing one. Rank by cost, cite the Law each is grounded in, and stop there: which one ships is the user's call, never auto-selected even when the cheapest option is obviously correct. When step 3 ran, this is normally the single design it converged on, not a fresh set of alternatives.
