...
export function TelegramToggle({ jobId, value = 'off' }: { jobId: string; value?: State }) {
  ...
  function startHold() {
    setHolding(true);
    timer.current = setTimeout(() => persist('retroactive'), 1500);
  }
  function cancelHold() {
    setHolding(false);
  ...
  return <button type="button" aria-label={`Telegram delivery ${state}`} aria-pressed={state !== 'off'} onClick={(e) => { e.preventDefault(); persist(state === 'off' ? 'on' : 'off'); }} onPointerDown={startHold} onPointerUp={cancelHold} onPointerLeave={cancelHold} className={`relative rounded-md border p-2 transition-ui ${state === 'off' ? 'border-line text-muted' : 'border-signal text-signal'} ${holding ? 'doc-telegram-hold' : ''}`}><Send className="h-4 w-4" /></button>;
}
