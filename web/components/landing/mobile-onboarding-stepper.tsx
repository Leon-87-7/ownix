...
export function MobileOnboardingStepper() {
  ...
  return (
    <div className="sm:hidden">
              ...
              <li key={item.id}>
                <button
                  ...
                  className={`flex min-h-11 w-full flex-col items-start justify-between rounded-md border px-3 py-2 text-left transition-ui focus:outline-none focus:ring-2 focus:ring-signal focus:ring-offset-2 focus:ring-offset-surface ${
                    isActive
                      ? 'border-signal bg-signal text-onsignal'
                      : 'border-line bg-canvas text-body hover:bg-raised hover:text-ink'
                  }`}
                >
      ...
      >
        <div
          className="h-full bg-signal transition-[width] duration-200 ease-out-quart motion-reduce:transition-none"
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <button
          ...
          className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-signal px-4 text-button font-medium leading-none text-onsignal transition-ui hover:bg-signal-bright active:bg-signal-deep active:scale-[0.98] motion-reduce:active:scale-100"
        >
