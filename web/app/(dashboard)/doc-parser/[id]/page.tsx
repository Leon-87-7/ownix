...
function outputFilename(job: Job, output: Output) {
  const stem =
    (job.title || job.id).replace(FILENAME_FORBIDDEN, '_') || job.id;
...
async function fetchOutputContent(output: Output) {
  const accept =
    output.kind === 'raw_txt' ? 'text/plain' : 'text/markdown';
  const res = await fetch(output.content_url, {
    headers: { Accept: accept },
  });
  if (!res.ok)
    throw new Error(`Output request failed (${res.status})`);
  return res.text();
}
