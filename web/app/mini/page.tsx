...
export default function MiniAppPage() {
  ...
  const [message, setMessage] = useState(
    'Verifying Telegram launch data…',
  );
  ...
  useEffect(() => {
    ...
    if (!initData) {
      setState('error');
      setMessage(
        'Open this from the Telegram bot button so Ownix can verify your account.',
      );
      return;
    }
    ...
    fetch('/api/auth/miniapp/session', {
      ...
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(
            body.detail || 'Telegram verification failed.',
          );
        if (!cancelled) {
          setConnectUrl(
            body.google_connect_url || '/api/google/connect',
          );
          setState('ready');
          setMessage(
            'Telegram verified. Connect Google to let Ownix create Drive and Sheets outputs for your Index.',
          );
        }
      })
      .catch((error: Error) => {
  ...
  function connectGoogle() {
    ...
    if (window.Telegram?.WebApp?.openLink) {
      ...
      window.Telegram.WebApp.openLink(
        new URL(connectUrl, window.location.origin).href,
      );
      return;
    }
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <h1 className="text-balance text-2xl font-semibold tracking-[-0.02em]">
                Connect Google inside Ownix
              </h1>
            </div>
            <span className="rounded-md border border-telegram-blue/40 bg-telegram-blue/10 px-2 py-1 font-mono text-mono-label text-telegram-blue">
              TG
            </span>
          </div>
