...
async def run(job: dict) -> None:
    ...
    # 2. Transcript acquisition — fetched before vision so the Gemini summary can be
    # grounded in what was actually said, not guessed from frames alone (ADR-0020).
    transcript_text, template_analysis = await _acquire_and_persist_transcript(
        job, url, chat_id, tag, frame_resp.get("title", ""), job.get("template")
    )
    ...
    # 4. Brave Search enrichment (opt-in)
    if links:
    ...
    # 5. Upload analysis markdown to Drive
    ...
    # 7+8. Send best frame photo, then links message (its message_id wins as anchor)
    ...
    # 9. Sheets logging (fire-and-forget)
    ...
    # -------------------------------------------------------------------------
    # Phase 2 (ADR-0020): template enrichment + transcript doc delivery — always last
    # -------------------------------------------------------------------------
    await _deliver_transcript_phase(
        job, chat_id, tag, title, platform, video_id, transcript_text, template_analysis
    )
...
async def _acquire_and_persist_transcript(
    job: dict, url: str, chat_id: int, tag: str, title: str, template: str | None,
) -> tuple[str | None, dict | None]:
    """Fetch + persist the transcript up front, before vision runs, so the Gemini
    summary can be grounded in what was actually said (ADR-0020 phase moved earlier)."""
    job_id = job["id"]
...
async def _deliver_transcript_phase(
    job: dict,
    chat_id: int,
    tag: str,
    title: str,
    ...
    """Run template enrichment and deliver the transcript .md doc — always last."""
    job_id = job["id"]
