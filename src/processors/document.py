...
async def run(job: dict, *, skip_document: bool = False) -> None:
    ...
    enrich_cost = CostContext(chat_id=chat_id, job_id=job_id, operation="document_enrich")
    raw = await generate(
        _build_document_prompt(text, job.get("freestyle_prompt")),
    ...
    summary_cost = CostContext(chat_id=chat_id, job_id=job_id, operation="document_summary")
    summary_md = await generate(summary_prompt, model="gemini-2.5-flash", cost=summary_cost)
