...
- **API** — `src/main.py` (uvicorn `src.main:app`). Hosts the Telegram webhook (`src/telegram/webhook.py` — route registration and update fan-out only; handlers live in `src/telegram/callbacks.py`, `commands.py`, `routing.py`, with shared plumbing in `context.py`), the ops-bot webhook (`/webhook/ops`, handlers in `src/telegram/ops.py` over the transport and policy helpers in `src/services/ops_bot.py`), `/health`, and the dashboard JSON API (`src/api/` — jobs, brain, spaces, parsed/doc-parser, preview, controls, auth, google_oauth). Auth is session-cookie middleware in `src/auth/`.
...
- **State** — SQLite WAL at `data/jobs.db`. `src/database.py` is the public facade; the implementation is one module per domain under `src/db/` (`core` opens every connection, `schema` holds the only DDL, then `jobs`, `users`, `tags`, `spaces`, `newsletter`, `templates`, `cache`, `admin`). Migrations run automatically at startup via `PRAGMA user_version`; each step in `src/db/migrations/steps.py` declares its own target version with `@migration(N)` or `sql(N, [...])` and is either a list of idempotent SQL statements or an async callable. Add a new one with the next number — file position carries no meaning, and a duplicate or missing version fails at import. Job status FSM: `pending → processing → transcript_done → enriching → done` (or `error` / `cancelled`); short/article/document skip the intermediate states.
...
Frontend design and component-layout conventions for `web/` live in `web/CLAUDE.md` (loads only when working under `web/`).
